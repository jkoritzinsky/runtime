...
#define LookupByKeyOrMiss(map, key, keymsg, ...)                                                                       \
    ...
    {                                                                                                                  \
        ...
        if (index == -1)                                                                                               \
            LogException(EXCEPTIONCODE_MC,                                                                             \
                         "SuperPMI assertion failed (missing key \"" #key "\" in map " #map ")" keymsg,                \
                         ##__VA_ARGS__);                                                                               \
...
struct FilterSuperPMIExceptionsParam_CaptureException
{
    ...
    FilterSuperPMIExceptionsParam_CaptureException() : exceptionCode(0), exceptionMessage(nullptr) {}
...
extern void RunWithErrorExceptionCodeCaptureAndContinueImp(void* param,
                                                           void (*function)(void*),
...
void RunWithErrorExceptionCodeCaptureAndContinue(LambdaTry function, LambdaFinally finally)
{
    ...
    RunWithErrorExceptionCodeCaptureAndContinueImp(
        &lambdaArgs, [](void* pParam) { ((LambdaArguments*)pParam)->pTryLambda->_lambda(); },
        [](void* pParam, DWORD exceptionCode) { ((LambdaArguments*)pParam)->pFinallyLambda->_lambda(exceptionCode); });
}
