    ...
    public class StructMarshallingGenerator : IIncrementalGenerator
    {
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            context.RegisterSourceOutput(
                    structMarshallingContextProvider
                    .Where(data => data.Context.MarshallingFeatures.IsBlittable)
                    .Select((data, ct) => PrintBlittableAttributeApplication(data.Context.Namespace, data.Syntax))
                    .Select((syntax, ct) => syntax.NormalizeWhitespace().ToFullString())
                    .Collect()
                    .Select((generatedSources, ct) =>
                    {
                        StringBuilder source = new();
                        // Mark in source that the file is auto-generated.
                        source.AppendLine("// <auto-generated/>");
                        foreach (string generatedSource in generatedSources)
                        {
                            source.AppendLine(generatedSource);
                        }
                        return (source: source.ToString(), diagnostics: ImmutableArray<Diagnostic>.Empty);
                    }),
                (context, data) =>
                {
                    foreach (Diagnostic diagnostic in data.diagnostics)
                    ...
                    context.AddSource("GeneratedStructMarshalling.Blittable.g.cs", data.source);
                });
        }
        ...
        private static MemberDeclarationSyntax PrintBlittableAttributeApplication(
            string? @namespace,
            StructDeclarationSyntax originalSyntax)
        {
            // Create stub function
            MemberDeclarationSyntax rootDeclaration = CreateTypeDeclarationWithoutTrivia(originalSyntax)
                .AddAttributeLists(AttributeList(
                    SingletonSeparatedList(
                        Attribute(
                            IdentifierName(TypeNames.BlittableTypeAttribute)))));

            bool addNativeMarshallingAttribute = true;
            foreach (TypeDeclarationSyntax containingType in originalSyntax.Ancestors().OfType<TypeDeclarationSyntax>())
            {
